              ...
              {visibleSections.map((section) => {
                ...
                return (
                  <div key={section.key} className="px-1">
                    <button
                      type="button"
                      className={`h-10 w-full text-sm border border-transparent rounded-none ${selectedKey === section.key ? 'font-bold text-slate-900 bg-slate-50' : 'font-medium text-slate-700'} hover:bg-slate-50`}
                      ...
                      {children.map((child, idx) => {
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            disabled={isDisabled}
                            className={`w-full px-2 py-1.5 text-sm border border-transparent rounded-none text-left ${selectedKey === targetKey ? 'font-bold text-slate-900 bg-slate-50' : 'font-medium text-slate-700 bg-white'} ${isDisabled ? 'opacity-40 cursor-not-allowed' : 'hover:bg-slate-50'}`}
